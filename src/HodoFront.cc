...
HodoFront::HodoFront(G4String name) : G4VSensitiveDetector(name){
  //runnum = 0;
  name = "HodoFront_log";
}
...
void HodoFront::Initialize(G4HCofThisEvent* HCE){
  runnum = 0;
  TrackID = 0;
  ParentID = 0;
  Counter = 0;
...
G4double HodoFront::GetEdep(){return EdepThisEventUnquenched;}
G4bool HodoFront::HadPrimary(){return HazPrimary;}
...
G4bool HodoFront::ProcessHits(G4Step* theStep, G4TouchableHistory*){
...
void HodoFront::EndOfEvent(G4HCofThisEvent*)
{
...
G4double HodoFront::ApplyBirksQuench(G4double En){
...
G4double HodoFront::EvaluateBirks(G4double En){
  if(En!=0){
    G4bool Interp = false;
    G4double EnOrig;
    if(En<0.1*keV){
