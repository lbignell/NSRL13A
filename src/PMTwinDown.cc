...
PMTwinDown::PMTwinDown(G4String name) : G4VSensitiveDetector(name){
  //runnum = 0;
  name = "PMTwinDown_log";
}

PMTwinDown::~PMTwinDown(){;}
...
void PMTwinDown::Initialize(G4HCofThisEvent* HCE){
...
vector< vector< double > >& PMTwinDown::GetEnSpec(){return PhotEn;}
vector< vector< double > >& PMTwinDown::GetParentIDs(){return ParentID;}
vector< vector< double > >& PMTwinDown::GetMeasEnSpec()
{return PMTFirePhotEnIn_nm;}
vector< vector< double > >& PMTwinDown::GetMeasParentIDs()
{return PMTFireParentID;}
G4double PMTwinDown::GetTotalPhotons(){return TotPhotons;}
G4double PMTwinDown::GetMeasPhotons(){return MeasPhotons;}
G4double PMTwinDown::GetPhotsOnPhotoCathode(){return PCphotons;}
...
G4bool PMTwinDown::ProcessHits(G4Step* theStep, G4TouchableHistory*){
  ...
  if(theStep->GetTrack()->GetParticleDefinition()->GetParticleName()
	...
	if(theStep->GetTrack()->GetCreatorProcess()->GetProcessName()
	  ...
	  PhotEn.at(1).push_back(theStep->GetPostStepPoint()
				 ->GetKineticEnergy());
	  ...
	  //Print a warning.
	  G4cout<<"WARNING: a photon was incident on the downstream PMT from an"
...
void PMTwinDown::EndOfEvent(G4HCofThisEvent*)
{
...
G4bool PMTwinDown::PMTHazFired(G4double Lambda){
