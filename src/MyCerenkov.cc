...
//
//
// $Id: MyCerenkov.cc 71478 2013-06-17 07:49:29Z gcosmo $
//
////////////////////////////////////////////////////////////////////////
...
// MyCerenkov::operator=(const MyCerenkov &right)
// {
// }
...
MyCerenkov::MyCerenkov(const G4String& processName, G4ProcessType type)
...
// MyCerenkov::MyCerenkov(const MyCerenkov &right)
// {
// }
...
G4bool MyCerenkov::IsApplicable(const G4ParticleDefinition& aParticleType)
{
    G4bool result = false;
...
void MyCerenkov::BuildPhysicsTable(const G4ParticleDefinition&)
{
...
//
G4VParticleChange*
MyCerenkov::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
...

{
  ...
  G4cout << "Running MyCerenkov PostStepDoIt()" << G4endl;
	...
	G4double t0 = pPreStepPoint->GetGlobalTime();
	  ...
	  G4cout << "NO MATERIALS PROPERTY TABLE!! "
		 << "Material = " << aMaterial->GetName() << G4endl; 
	...
	G4MaterialPropertyVector* Rindex = 
        ...
        if (!Rindex){
	  G4cout << "No RINDEX registered!! Material = "
	...
	G4cout << "MeanNumberOfPhotons = " << MeanNumberOfPhotons << G4endl;
	...
	if (verboseLevel>0) {
	   G4cout <<"\n Exiting from MyCerenkov::DoIt -- NumberOfSecondaries = "
...
void MyCerenkov::BuildThePhysicsTable()
{
...
G4double MyCerenkov::GetMeanFreePath(const G4Track&,
                                           G4double,
                                           G4ForceCondition*)
{
        return 1.;
...
G4double MyCerenkov::PostStepGetPhysicalInteractionLength(
                                           const G4Track& aTrack,
                                           G4double,
                                           G4ForceCondition* condition)
{
  G4cout << "Calculating Cerenkov Physical Interaction Length" << G4endl;
        *condition = NotForced;
	...
	if(!(*thePhysicsTable)[materialIndex]) { return StepLimit; }
	   ...
	   G4cout << "Rindex max value = " << nMax << G4endl;
        } else {
	  G4cout << "No Refractive index found!!" << G4endl;
           return StepLimit;
        }
        ...
        if (gamma < GammaMin ){
	  G4cout << "Particle not going fast enough for Cerenkov!" << G4endl;
	  G4cout << "Calculating how many photons *would* have been made"
		 << G4endl;
	  const G4double thecharge = aParticle->
                                   GetDefinition()->GetPDGCharge();
	  G4double MeanNumberOfPhotons = 
	    GetAverageNumberOfPhotons(thecharge,beta,aMaterial,Rindex);
...
G4double 
MyCerenkov::GetAverageNumberOfPhotons(const G4double charge,
                              const G4double beta, 
			      const G4Material* aMaterial,
			      G4MaterialPropertyVector* Rindex) const
{
  G4cout << "Getting Number of photons" << G4endl;
	...
	// the GetValue() method of G4PhysicsVector.  
		...
		if (verboseLevel>0) {
			...
			G4cout << "CAImax = " << CAImax << G4endl;
			G4cout << "ge = " << ge << G4endl;
			G4cout << "Pmin = " << Pmin << G4endl;
			G4cout << "Pmax = " << Pmax << G4endl;
			G4cout << "dp = " << dp << G4endl;
			G4cout << "NumPhotons = "
			       << Rfact * charge/eplus * charge/eplus *
